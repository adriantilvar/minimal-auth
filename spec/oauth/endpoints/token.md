...
| scope         | RECOMMENDED / REQUIRED | The scope of the access, expressed as a list of space-delimited, case-sensitive strings. If the value contains multiple spaced-delimited strings, their order does not matter request. Recommended if identical to the scope requested by the `client`, required otherwise                        |
